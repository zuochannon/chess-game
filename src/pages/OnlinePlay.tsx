...
export function OnlinePlay() {
	...
	useEffect(() => {
		...
		const interval = setInterval(() => {
			fetch(
				`${import.meta.env.VITE_SERVER}/onlinePlay/${roomid}/getMoves`,
				{
					method: "GET",
					credentials: "include",
					headers: {
				...
				if (response.status === 200) {
					...
					if (moveHistory.length > data.moves.length) {
						...
						setMoveHistory([]);
						onlineHandler.restartGameOnline(
							onlineHandler.restartGame
						);
					}
				}
			});
